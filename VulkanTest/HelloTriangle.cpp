...
void HelloTriangle::createTextureSampler() {
	...
	//deal with oversampling, more fragments than texels
	samplerInfo.magFilter = VK_FILTER_LINEAR;
	//deal with undersampling, more texels than fragments
...
void HelloTriangle::createIndexBuffer() {
	...
	createBuffer(bufferSize, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
...
void HelloTriangle::createBuffer(VkDeviceSize size, VkBufferUsageFlags flags, VkMemoryPropertyFlags properties,
...
void HelloTriangle::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling,
...
std::vector<char> HelloTriangle::readFile(const std::string& filename) {
	...
	file.close();
	std::cout << "name " << filename << "\t" << "size " << fileSize << "\n";
	return buffer;
}
